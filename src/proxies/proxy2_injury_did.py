...
ROOT = Path(__file__).resolve().parents[2]  # now /files/Conor_Keenan_Project
...
def load_panel() -> pd.DataFrame:
    ...
    Expected columns in panel_injury.parquet:
      match_id, team_id, player_id, date, season, opponent_id, xpts, unavailable
    """
...
def filter_player_seasons(
    summary: pd.DataFrame, min_unavail: int = 2, min_avail: int = 2
) -> pd.DataFrame:
    """
...
def estimate_did_for_player_season(
    df: pd.DataFrame, pid, tid, season
) -> dict | None:
    """
...
def run_did(
    ...
    """
    good_ps = filter_player_seasons(summary, min_unavail=min_unavail, min_avail=min_avail)
